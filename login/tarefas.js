...
function criarTarefas(tarefas) {
    ...
    for(let i = 0; i < tarefas.length; i++) {
        ...
        if(id == localStorage.getItem('IdUsuario')){
            ...
            tarefaId.appendChild(tarefaIdSpan)
    ...
    const dica_alterar = document.createElement('p')
    dica_alterar.textContent = 'Clique em uma tarefa para altera-la'
    const dica_adicionar = document.createElement('p')
    dica_adicionar.textContent = 'Clique no icone para adicionar mais uma tarefa'

    adicionarTarefa.replaceChildren(dica_alterar, dica_adicionar)
...
// Uso:
//excluirTarefaPorId(9); // Exclui a tarefa com ID 6
